...
import { $, rdb, yadcf, empty, BREAKPOINTS, COUNTRIES, DT_LOADING, DTHelpers, Helpers, ksort } from 'Utils';
 ...
 * @type {FrontPage}
 */
class FrontPage extends DTHelpers {
  /**
   * Primary constructor.
   *
   * @since 1.0.0
    ...
    const table = this.$table.DataTable({ // eslint-disable-line
      ...
      ajax: {
        ...
        dataSrc: ( json ) => {
          ...
          // Parse response.
          each( json.data, ( match ) => {
            const api = {
              ID: match.ID,
              idStr: `match-${ match.ID }`,
              competition: {
                display: DTHelpers.competition( match ),
                filter: ! empty( match.competition.label ) ? match.competition.label : match.competition.name
              },
              date: {
                matchID: match.ID,
      ...
      columns: [
        ...
        },
        {
          ...
          render: function( data, type, row ) {
            if ( 'display' === type ) {
              return DTHelpers.formatDate( data.matchID, data.display, data.permalink );
            } else if ( 'filter' === type ) {
              return data[ type ];
            }
      ...
      drawCallback: function( settings ) {
        ...
        $teamFilters.on( 'change', 'input[ name="wpcm_team" ]', function( e ) {
          ...
          const checkedBoxes = compact( $teamFilters.find( '.active' ).map( function() {
            return this.id;
          }).get() );
      ...
      initComplete: function() {
        ...
        self.$win.on( 'resize orientationchange', throttle( api.draw, 300 ) );
  ...
  _venueOptions( responseData ) {
    ...
    each( responseData, ( match ) => {
      if ( 'GB' === match.venue.schema.addressCountry ) {
        const country = DTHelpers.flag( match.venue.schema );
    ...
    each( responseData, ( match ) => {
      if ( 'GB' === match.venue.schema.addressCountry ) {
        ...
        if ( ! includes( venueGroup[ country[ 0 ] ], match.venue.name ) ) {
          venueGroup[ country[ 0 ] ].push( match.venue.name );
        }
      } else if ( ! includes( venueGroup[ COUNTRIES[ match.venue.schema.addressCountry.toUpperCase() ] ], match.venue.name ) ) {
    ...
    each( responseData, ( match ) => {
      if ( 'GB' === match.venue.schema.addressCountry ) {
        ...
        venueGroup[ country[ 0 ] ] = sortBy( venueGroup[ country[ 0 ] ], venue => venue.toLowerCase() );
      } else {
        venueGroup[ COUNTRIES[ match.venue.schema.addressCountry.toUpperCase() ] ] = sortBy( venueGroup[ COUNTRIES[ match.venue.schema.addressCountry.toUpperCase() ] ], venue => venue.toLowerCase() );
      }
    });
    ...
    each( venueGroup, ( venues, country ) => {
  ...
  static _radioFilters( checkedBoxes ) {
    ...
    if ( xor( checkedBoxes, teams ).length === 0 ||
