 ...
 * @since 1.2.1 - Modified `$trans_key` value. No longer using MD5.
 *
...
function rdb_remote_get( $url, $assoc_array = false, $args = array() ) {
    $url_path  = untrailingslashit( wp_parse_url( $url, PHP_URL_PATH ) );
    $trans_key = sanitize_title( preg_replace( '/^\//', '', $url_path ) );
...
function _rdb_array_attrs( array $attributes, bool $make_them_data = false ) {
    ...
    foreach ( $attributes as $name => $value ) {
        ...
        if ( is_bool( $value ) && $value ) {
            ...
            $pairs[] = sprintf( '%1$s="%2$s"', $name, $value );
        }
    }
 ...
 *     @type WP_Term[] $options     Dropdown options.
...
function rdb_dropdown_menu( $args = '' ) {
    ...
    if ( ! empty( $args['options'] ) ) {
        ...
        foreach ( $args['options'] as $term ) {
            if ( $term->parent > 0 ) {
                ...
                $option_name = sprintf( '%1$s - %2$s', $parent->name, $term->name );
            } else {
    ...
    if ( $args['echo'] ) {
        printf(
            '<select id="%1$s" class="%2$s" name="%3$s" placeholder="%4$s">%5$s</select>%6$s',
            esc_attr( $args['id'] ),
            esc_attr( $args['class'] ),
            esc_attr( $args['name'] ),
...
function rdb_input_box( $args = '' ) {
    ...
    if ( $args['echo'] ) {
        printf(
            '<input id="%1$s" name="%2$s" placeholder="%3$s" type="%4$s" value="" />%5$s',
            esc_attr( $args['id'] ),
            esc_attr( $args['name'] ),
            esc_attr( $args['placeholder'] ),
            esc_attr( $args['type'] ),
        ...
        return '<input ' . _rdb_attr_value( $args ) . ' />' . wp_kses_post( $icon );
    }
}
...
function _rdb_rename_uploaded_player_images( $filename, $filename_raw ) {
    ...
    $player_badge = absint( get_post_meta( $post->ID, 'wpcm_number', true ) );
...
function rdb_svg_list() {
    ...
    /**
     * Filters the list of SVG images.
     *
     * @since 1.0.0
     *
     * @param array List of known SVGs.
     */
    return apply_filters( 'rdb_svg_list', $svg_list );
}
 ...
 */
function rdb_get_svg( $icon, $attrs = '' ) {
    $rdb_svg_list = rdb_svg_list();
     ...
     */
    $svg = array(
        '<svg ' . _rdb_attr_value( $attrs ) . '>',
            '<use xlink:href="#' . esc_attr( $icon ) . '" href="#' . esc_attr( $icon ) . '" />',
        '</svg>',
    );
